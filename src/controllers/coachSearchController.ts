...
export const searchCoaches = async (req: Request, res: Response) => {
  try {
    ...
    let response1 = await axios.get(
      `${process.env.API_GATEWAY_URL}/auth/profile/userInfo`,
      {
        params: { userQuery }
      }
    );

    let coaches = response1.data;

    const coachIds = coaches.map((c: { _id: any; }) => c._id);
    ...
    if (type) {
      ...
      const response = await axios.get(`${process.env.API_GATEWAY_URL}/auth/profile/getCoach_Id_spl`, {
        params: {
          coachIds,
          type
        }
      ...
      let coachesWithType = response.data;

      filteredCoachIds = coachesWithType.map((c: { userId: any; }) => c.userId);
    }
    ...
    const response = await Promise.all(
      paginatedCoaches.map(async (coach: { _id: any; firstName: any; lastName: any; email: any; profileImage: any; }) => {
        // Get coach details
        // const coachDetails = await CoachInfo.findOne({ userId: coach._id }).lean();


        let response2 = await axios.get(
          `${process.env.API_GATEWAY_URL}/auth/profile/coach_by_userId`,
          {
            params: { userId: coach._id }
          }
        );

        let coachDetails = response2.data;
